...
/**
 * Custom JavaScript for the mockinvoice categories section
 *
...
?>      <!-- Custom -->
   <script src="<?php echo ROOT_RELATIVE_PATH; ?>assets-system/js/form-validation.js"></script>
      ...
      EditableTable = {
            ...
            this.datatable = this.$table.DataTable({
              "autoWidth": false,
                  ...
                  "columns": [
                     ...
                     { "name": "Description", "data": "description", "orderable": true, "render": function( data ) { if ( data ) { return data.replace( /\n/g, "<br />" ); } }, "className": "textarea-output", "defaultContent": "" },
                     {"name": "Actions", "data": "actions", "sClass": "actions", "orderable": false,"defaultContent": ""},
                  ],
                  "ajax": '/admin/mockinvoice-description/get-rows-json'
            });
                  ...
                  switch(i){
                     ...
                     case 0:
                        if ( data['description'] == undefined ) {
                           $this.html( '<div class="form-group"><textarea rows="1" wrap="physical" required="required" data-msg-required="Description is required"  name="description" class="form-control input-block textarea-output"></textarea></div>' );
                        } else {
                           $this.html( '<div class="form-group"><textarea rows="1" wrap="physical" required="required" data-msg-required="Description is required"  name="description" class="form-control input-block textarea-output">' + data['description'] + '</textarea></div>' );
                        }
                     break;
                     case 1:
                  ...
                  this.$addButton.removeAttr( 'disabled' );
                   var data_rows = this.datatable.row( $row.get( 0 ) ).data(); // hold the data for the whole row
                     ...
                     $.each( data_rows, function( key, value ) {
                        switch(key){
                              case 'id':
                                 holder += 'mockInvoiceDescriptionId=' + value;
                              break;
                           }
                     });
                     values = $row.find( 'td' ).map(function() {
         ...
         rowRemove: function( $row ) {
            ...
            if ( delete_rows.id != undefined ) {
               var talbe_id = ( 'mockInvoiceDescriptionId=' + delete_rows.id ); 
