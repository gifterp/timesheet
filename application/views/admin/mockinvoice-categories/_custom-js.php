      ...
      EditableTable = {
            ...
            this.datatable = this.$table.DataTable({
              "autoWidth": false,
                  ...
                  "columns": [
                     ...
                     {"name": "Categories", "data": "category", "orderable": true,"defaultContent": ""},
                     {"name": "Actions", "data": "actions", "sClass": "actions", "orderable": false,"defaultContent": ""},
                  ],
                  "ajax": '/admin/mockinvoice-categories/get-rows-json'
            });
                  ...
                  switch(i){
                     ...
                     case 0:
                        if ( data['category'] == undefined ) {
                           $this.html( '<div class="form-group"><input type="text" maxlength="30" name="category" value="" required="required"  data-msg-required="Category is required"  class="form-control"></div>' );
                        } else {
                           $this.html( '<div class="form-group"><input type="text"  name="category"  class="form-control input-block " data-val-required maxlength="30" required="required"  data-msg-required="Category is required" value="' + data['category'] + '"/></div>' );
                        }
                     break;
                     case 1:
                  ...
                  this.$addButton.removeAttr( 'disabled' );
                   var data_rows = this.datatable.row( $row.get( 0 ) ).data(); // hold the data for the whole row
                     ...
                     $.each( data_rows, function( key, value ) {
                        switch(key){
                              case 'id':
                                 holder += 'mockInvoiceCategoryId=' + value;
                              break;
                           }
                     });
                     values = $row.find( 'td' ).map(function() {
         ...
         rowRemove: function( $row ) {
            ...
            if ( delete_rows.id != undefined ) {
               var talbe_id = ( 'mockInvoiceCategoryId=' + delete_rows.id ); 
